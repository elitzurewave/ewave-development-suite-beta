...
const { scanContent, detectContentType, parseFrontmatter } = require('../lib/security-patterns');
...
const CONFIG_PATH = path.join(PLUGIN_DIR, 'scripts', 'config', 'security-gate-config.json');
const PROTECTED_DIRS = ['agents', 'skills', 'commands', 'rules', 'hooks', 'scripts'];
...
function loadConfig() {
  try {
    ...
    console.error('[SECURITY GATE] Warning: Could not load config, using defaults:', err.message);
    return {
...
function writeAuditLog(config, entry) {
  try {
    let logPath = path.resolve(PLUGIN_DIR, config.auditLogPath || 'logs/security-audit.jsonl');
...
function runHookMode() {
  ...
  process.stdin.on('end', () => {
    ...
    try {
      ...
      const relativePath = path.relative(PLUGIN_DIR, path.resolve(filePath)).replace(/\\/g, '/');
      ...
      if (criticals.length > 0) {
        console.error('');
        console.error(`[SECURITY GATE] BLOCKED - ${criticals.length} critical finding(s):`);
        console.error('');
        ...
        console.error('');
        console.error('  To override: add the pattern ID(s) to allowedPatterns in');
      ...
      if (highs.length > 0) {
        console.error('');
        console.error(`[SECURITY GATE] WARNING - ${highs.length} finding(s) in ${path.basename(filePath)}:`);
        console.error('');
      ...
      if (mediums.length > 0 && highs.length === 0) {
        console.error('');
        console.error(`[SECURITY GATE] INFO - ${mediums.length} note(s) in ${path.basename(filePath)}:`);
      ...
      if (contentToScan && contentToScan.length > 0) {
        console.error('[SECURITY GATE] BLOCKED: Scanner error on non-empty content. Review manually.');
        process.exit(1);
      }
...
function runScanMode(targetDir) {
  ...
  const scanRoot = targetDir
    ? path.resolve(PLUGIN_DIR, targetDir)
    : PLUGIN_DIR;

  // Prevent path traversal outside plugin directory
  if (!scanRoot.startsWith(PLUGIN_DIR)) {
    console.error('[SECURITY GATE] Error: Scan directory must be within the plugin directory.');
    process.exit(1);
  }
  ...
  console.log(`CRITICAL: ${criticals.length} | HIGH: ${highs.length} | MEDIUM: ${mediums.length}`);
  console.log('');
...
function runCheckPathMode(filePath) {
  ...
  try {
    ...
    if (result.findings.length === 0) {
      ...
      if (criticals.length > 0) {
        console.error('');
        console.error(`[SECURITY GATE] POST-SCAN ALERT - ${criticals.length} CRITICAL finding(s) in ${path.basename(resolved)}:`);
        console.error('');
        for (const f of criticals) {
          console.error(`  ${f.id}  Line ${f.line}: "${f.match}"`);
          console.error(`          ${f.description}`);
